...
(janitorial, grounds, security guards/patrol, facilities support), forked
from the BidYard Bid* pattern — see README.md for what it is and current
build status.
...
This repo has its own git history, separate from BidYard, BidHawk, and
BidPulse. Do not add a sibling-repo sync workflow without being asked.

## Default loop

1. Edit in this repo only. Do not read from or write to sibling Bid*
   products unless Ben asks — this product's point is a clean IP boundary.
...
- Shared package: `@netacracy/bid-core`.

## Do not

- Reuse another Bid* product's Supabase credentials, API keys, or
  `.env.local` — BidKeep needs its own.
- Invent SCA wage-determination numbers or DOL rates. Surface WD links
  only when SAM.gov (or a later wired source) provides them.
- Reintroduce drone / Blue UAS / construction-trade marketing or NAICS
  (236/237/238). The facilities trust page is `/set-asides`.
- Reintroduce a title-keyword relevance filter or wide-net keyword search
  pass on top of NAICS/ALN matching for SAM.gov. Facilities NAICS codes
  name the work directly.
- Hardcode a live Stripe Payment Link URL. Keep
  `NEXT_PUBLIC_STRIPE_PAYMENT_LINK_URL` env-driven.
