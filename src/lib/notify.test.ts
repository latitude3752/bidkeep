...
describe("notifyNewOpportunities / notifyNewGrantAwards / notifyNewFundingOpportunities", () => {
  const opportunity = {
    title: "Custodial Services Barracks Building 412",
  ...
  const grantAward = {
    recipientName: "STATE OF FLORIDA",
    awardingAgency: "Department of Transportation",
    amount: 23636511,
    state: "Florida",
    programNumber: "20.205",
  };
  ...
  it("sends the digest and returns null on success", async () => {
    ...
    expect(arg.html).toContain("STATE OF FLORIDA");
  });
  ...
  it("sends the funding-opportunity digest with a working link and close date", async () => {
    ...
    expect(arg.html).toContain("ALN 20.205");
  });
