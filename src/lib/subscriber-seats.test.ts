...
describe("subscriber-seats", () => {
  ...
  it("allows a second company to sign up independently", async () => {
    ...
    await createSeat({ email: "a@acme.test", name: "A", company: "Acme Drones" });
  ...
  it("caps seats per company, independently of other companies", async () => {
    const { createSeat } = await import("./subscriber-seats");
    for (let i = 0; i < 5; i++) {
      await createSeat({ email: `seat${i}@acme.test`, name: `Seat ${i}`, company: "Acme Drones" });
    }
    await expect(
      createSeat({ email: "overflow@acme.test", name: "Overflow", company: "Acme Drones" })
