 ...
 * procurement sourcing, since a lot of janitorial, grounds, and guard
 * work is bought by cities, school districts, and housing authorities
 * below the federal SAM.gov level the Bid* family engine covers.
 *
 ...
 * facilities-trade keywords as separate queries. Bonfire's API has no
 * NAICS filter, so a short keyword list is the only option.
 */

const BONFIRE_PREVIEW_BASE =
  "https://common-production-api-global.bonfirehub.com/v1.0/projects/preview/search";

/** Facilities-trade keywords run as separate free-text searches against
 ...
 * bounds total requests (9 keywords * up to 4 pages = up to 36 calls per
