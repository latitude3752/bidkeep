...
function SetAsideBadge({ raw }: { raw: string | null }) {
  const label = setAsideLabel(raw);
  const priority = isPrioritySetAside(raw);
  return (
    <span
      className={
        priority
          ? "inline-block rounded-full bg-gold-500/20 px-2 py-0.5 text-[11px] font-semibold uppercase tracking-wide text-gold-800"
          : "inline-block rounded-full bg-navy-950/5 px-2 py-0.5 text-[11px] font-medium text-ink/70"
      }
    >
      {label}
    </span>
  ...
  return (
    <article className="rounded-xl border border-navy-950/10 bg-white p-4">
      ...
      <dl className="mt-3 grid grid-cols-2 gap-x-3 gap-y-1 text-xs text-ink/70">
        ...
        <dd>
          <SetAsideBadge raw={op.setAsideType} />
        </dd>
